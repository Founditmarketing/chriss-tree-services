...
module.exports = async (req, res) => {
  ...
  if (!name || !email) {
    res.statusCode = 303;
    res.setHeader("Location", "/contact-us-2?error=missing");
    return res.end();
  }

  const to   = process.env.CONTACT_TO_EMAIL   || "ctservice20\@yahoo\.com";
  const from = process.env.CONTACT_FROM_EMAIL || "Chris's Tree Services <leads@chrisslawnandtreeservices.com>";

  const subject = `New lead from chrisslawnandtreeservices.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on chrisslawnandtreeservices.com`;
  const html = `
<h2>New lead from the Chris's Tree Services site</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on chrisslawnandtreeservices.com</p>`;
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      res.statusCode = 303;
      res.setHeader("Location", "/contact-us-2?error=send");
      return res.end();
    }
  } catch (e) {
